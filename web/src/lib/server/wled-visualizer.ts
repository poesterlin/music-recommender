...
class EmbeddingVisualizer {
	...
	private gridToColors(
		grid: Float32Array,
		hueOffset: number = 0,
		waveEffect: Float32Array | null = null
	): Uint8Array {
	...
	private async sendToWledUDP(colors: Uint8Array): Promise<void> {
		try {
			...
			// Use Node's dgram module in a child process (UDP from server runtimes)
	...
	public async visualizeCurrentTrack(): Promise<void> {
		try {
			...
			const embedding =
				track.embedding instanceof Float32Array
					? track.embedding
	...
	public async startContinuousVisualization(checkInterval: number = 15000): Promise<void> {
		...
		const checkAndUpdate = async () => {
			try {
				...
				if (!this.currentEmbedding || !this.arraysEqual(this.currentEmbedding, embedding)) {

<script lang="ts">
	...
	function edit(s: VibeSchedule) {
		...
		endHour = s.endHour;
		picked = new SvelteSet(s.clusterIds);
	...
	async function save() {
		...
		toastStore.show(editingId === null ? 'Slot added' : 'Slot updated');
		resetForm();
		await refresh();
	}
...
<section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
	...
	{#if schedules.length === 0}
		<p class="mt-2 text-sm text-gray-400">No slots yet.</p>
	{:else}
		<div class="mt-4 space-y-2">
			{#each schedules as s (s.id)}
				<div
					class="flex flex-wrap items-center gap-3 rounded-xl border px-4 py-3 {activeSchedule?.id ===
					s.id
				...
				>
					<div class="min-w-40 flex-1">
						<p class="font-bold text-gray-900">
							{s.name}
							{#if activeSchedule?.id === s.id}<span
									class="ml-1 text-xs font-semibold text-green-600">● now</span
								>{/if}
						</p>
...
<section
	id="slot-form"
	class="mt-6 scroll-mt-24 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm"
>
	<h2 class="text-lg font-bold text-gray-900">{editingId === null ? 'Add slot' : 'Edit slot'}</h2>
	<p class="mt-1 text-sm text-gray-500">Hours are 0–24 and can wrap overnight, so 22–6 works.</p>
	<div class="mt-4 grid max-w-2xl gap-3">
		...
		<div class="grid gap-1 text-sm text-gray-600">
			<span>Clusters in this slot ({picked.size} picked)</span>
			<div
				class="grid max-h-64 grid-cols-1 gap-1.5 overflow-y-auto rounded-xl border border-gray-200 p-2 sm:grid-cols-2"
			>
				{#each clusterIds as id (id)}
					<label
						class="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-gray-50 {picked.has(
							id
						)
					...
					>
						...
						/>
						<span class="truncate"
							><b class="font-normal text-gray-400">#{id}</b> {clusterName(id)}</span
						>
					</label>
				{/each}
			</div>
		...
		<div class="flex flex-wrap gap-2">
			...
			{#if editingId !== null}
				<button
					class="rounded-xl bg-gray-200 px-5 py-2.5 font-semibold text-gray-700 hover:bg-gray-300"
